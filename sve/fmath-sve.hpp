...
/*
	tz0 = exp(tz0)
	destroy tz1, tz2
	read expMin, expMax, log2, log2_e, expCoeff[5]
*/
void genExp1(Xbyak::CodeGenerator *gen, const PReg& allOne, const ZReg& tz0, const ZReg& tz1, const ZReg& tz2, const ZReg& expMin, const ZReg& expMax, const ZReg& log2, const ZReg& log2_e, const ZReg expCoeff[5])
{
	gen->fmin(tz0.s, allOne, expMax.s);
	gen->fmax(tz0.s, allOne, expMin.s);
	gen->fmul(tz0.s, tz0.s, log2_e.s);
	gen->frintn(tz2.s, allOne, tz0.s); // rounding : float -> float
	gen->fcvtzs(tz1.s, allOne, tz2.s); // float -> int
	gen->fsub(tz2.s, tz0.s, tz2.s);
	gen->fmul(tz2.s, tz2.s, log2.s);
	gen->movprfx(tz0.s, allOne, expCoeff[4].s);
	gen->fmad(tz0.s, allOne, tz2.s, expCoeff[3].s);
	gen->fmad(tz0.s, allOne, tz2.s, expCoeff[2].s);
	gen->fmad(tz0.s, allOne, tz2.s, expCoeff[1].s);
	gen->fmad(tz0.s, allOne, tz2.s, expCoeff[0].s);
	gen->fmad(tz0.s, allOne, tz2.s, expCoeff[0].s);
	gen->fscale(tz0.s, allOne, tz1.s); // tz0 *= 2^tz1
}
...
//		, logf_v(0)
	...
	{
		genExp1(this, p, tz0, tz1, tz2, expMin, expMax, log2, log2_e, expCoeff);
	}
