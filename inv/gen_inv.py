...
def gen_inv(N):
  ...
  with Function(f'invModPre{N}', k, py, px, pm):
    ...
    u_le_v = Label()
    exit_lp = Label()
    br(icmp('eq', x, 0), exit_lp, entry)

    L(entry)
    phi_k = k = phi(ZERO32, start)
    phi_s = s = phi(ONE, start)
    phi_r = r = phi(ZERO, start)
    phi_v = v = phi(x, start)
    phi_u = u = phi(m, start)
    ...
    phi_v.link(v, lp)
    phi_u.link(u, lp)
    br(icmp('eq', v, ZERO), exit_lp, entry)

    L(exit_lp)
    r = phi((ZERO, start), (r, lp))
    k = phi((ZERO32, start), (k, lp))
